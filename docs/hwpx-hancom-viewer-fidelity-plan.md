...
- `keepWithNext`와 `lineSegArray` 높이 힌트를 함께 사용해 페이지 분할 경계를 보정
- `1x1` 박스형 테이블은 내부 블록 단위로, 일반 다중 행 테이블은 안전한 행 경계 기준으로 분할
- 긴 문단은 줄 단위로 분해해 페이지 상단/하단에 최소 줄 수를 남기도록 widow/orphan 성격의 1차 규칙을 적용
- 문단은 logical line wrapper로 렌더링하고 실제 줄 높이를 기준으로 페이지 경계 계산을 보정
- floating 그림/도형은 reserved-height footprint를 계산해 페이지 경계에서 본문 reflow를 보정
...
- 복잡한 `rowSpan` 표를 한컴처럼 끊는 더 정교한 표 분할
- 좌우 `textWrap`까지 포함한 floating 개체의 본문 감싸기
- `keepLines`, widow/orphan, 줄 단위 조판을 한컴 조판 엔진 수준으로 맞추는 세부 규칙
